...
export default function PatientManagement() {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      // Use nurse-specific patient search endpoint
      const res = await api.get(`/nurses/patients/search?query=${searchQuery}`);
          ...
          <Tbody>
            {patients.map((patient) => (
              <Tr key={patient.patient_id}>
                ...
                <Td>
                  <Button size="sm" variant="outline" onClick={() => navigate(`/nurse/patient/${patient.patient_id}/history`)}>
                    View History
                  </Button>
                </Td>
              </Tr>
